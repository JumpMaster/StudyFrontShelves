...
PapertrailLogger::PapertrailLogger(String host, int port, LogLevel level, String system, String context) {
  mHost = host;
  mPort = port;
  mLevel = level;
  // mColor = color;
...
size_t PapertrailLogger::write(uint8_t c) {
  ...
  if (c == '\n' || mBufferPos == BUFFER_SIZE - 1) {
    ...
    String syslogMessage = "<" + String(FacilityCode*8 + mLevel) + ">1 - " + mSystem + " " + mContext + " - - - " + String((const char *) mBuffer);
